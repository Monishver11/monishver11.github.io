...
Before any more machinery, we need to fix what "fast" means, because in this domain it means at least six different things and they trade off against each other.

Start with the two families everything falls into. **Latency** is one request's experience: how long did *this* user wait, for the first token and for each token after it. **Throughput** is the operator's aggregate: how much total work the system completes per second, across all requests. They are measured from opposite ends of the system, they pull in opposite directions (we'll see exactly why shortly), and the six metrics below are refinements of these two, four latency metrics and two throughput metrics.

One more piece of vocabulary before the table: **percentiles**. Latency is never a single number; it's a distribution over many samples, and "p99" names a point on it: the value that 99% of samples fall below, so p99 ITL = 200 ms means 1 gap in every 100 was worse than 200 ms. The reason serving people obsess over p99 rather than the mean is that means hide exactly the failures that matter: a request that generates 500 tokens experiences 500 ITL samples, so even a "1 in 100" stall is nearly guaranteed to hit every long request a few times, and the user perceives the freeze, not the average. Tail percentiles are where scheduling pathologies live, which is why service targets are stated on them.
...
A word on the throughput row's two units, since they're not interchangeable either. **Tokens/s** counts the engine's actual work: a request that generates 900 tokens is 100x the work of one that generates 9, and tokens/s weighs them accordingly, which makes it the right unit for comparing engines and kernels. **Requests/s** is what capacity planning ultimately buys: "how many users can this replica serve." The two are related only through the workload's average token count per request, so converting between them requires fixing the workload, and a tokens/s number measured on short outputs says little about your long-output service. Two smaller conventions to check when reading any benchmark: whether "tokens" means output tokens only or prompt plus output (both get reported, and prefill-heavy workloads make the difference large), and always which workload produced the number.
...
The second tension deserves a slower walk, because it's the one that makes benchmark numbers misleading. **Any engine can buy throughput with batch size at the cost of latency.** The mechanism is the weight-read amortization from the numbers section: one decode step reads the weights once and produces one token per sequence in the batch, so doubling the batch nearly doubles tokens/s while memory-bound. But each individual user's tokens still arrive one per step, and steps get *slower* as the batch grows (more KV to read, more sampling work), so every user's ITL degrades as the operator's tokens/s improves. The same engine, on the same GPU, can report wildly different tokens/s depending on how much latency pain it is configured to inflict. That is why a headline tokens/s with no latency constraint attached is not comparable across systems: system A's 5000 tok/s at 300 ms ITL and system B's 3000 tok/s at 40 ms ITL aren't even measurements of the same trade-off point, and either system could probably match the other's number by moving along its own batch-size curve.

**Goodput** is the fix, and it's simpler than the name suggests: state the latency requirements first, then count only the requests that met *all* of them ([DistServe](https://arxiv.org/abs/2401.09670)). Concretely, with requirements "TTFT under 500 ms and TPOT under 50 ms," a request that finished but took 900 ms to first token contributes **zero**, exactly as if it had failed; goodput is the rate of requests contributing one. Push batch size up and raw throughput keeps rising, but past some point more and more requests blow their latency bounds and drop out of the count, so goodput rises, peaks, and then *falls*. That peak is the honest operating point, and its existence is the whole argument: goodput bakes the latency constraint into the throughput number, making it the one headline figure that is comparable across systems, which is why it, and the exact requirements behind it, is what to ask for when someone quotes a benchmark.

Those stated requirements have a name: **SLOs** (service-level objectives), the internal targets, which become an **SLA** when written into a contract with consequences. For an LLM service they're typically a TTFT bound and an ITL or TPOT bound, stated at a percentile (p99 TTFT under some threshold, not mean), sometimes plus E2EL for batch-style jobs. Where the values come from is the product, not the engine: an interactive chat product wants first tokens fast and a stream that outpaces reading; an offline summarization pipeline may have no per-token latency requirement at all, in which case the right move is to drop the ITL SLO and run huge batches for raw throughput. And once the SLOs are fixed, tuning has a well-defined objective rather than a vibe: raise load and batch knobs until the constrained percentile approaches its bound, i.e., operate at maximum goodput. The scheduling section's knobs are exactly the levers this procedure adjusts.
...
**Continuous batching** fixes this by re-forming the batch *every step*. This is Orca's iteration-level scheduling ([Yu et al., OSDI '22](https://www.usenix.org/conference/osdi22/presentation/yu)), and the name says exactly what changed: the unit the scheduler hands to the GPU shrinks from "a whole generation" to "one iteration," a single forward pass producing one token per sequence. After every iteration, control returns to the scheduler, which removes sequences that just finished and admits waiting ones before launching the next iteration. A finished request's slot is refilled at the very next step instead of idling until the batch drains, and a new arrival waits at most one step to start rather than one full batch.

The two diagrams below (from [Anyscale's continuous batching post](https://www.anyscale.com/blog/continuous-batching-llm-inference)) are the cleanest mental model I've seen for this. Read each grid as: one row per batch slot, one column per model step ($$T_1, T_2, \ldots$$), yellow cells are prompt tokens, blue cells are generated tokens, red is the end-of-sequence token, and a white cell is a step that slot spent doing nothing.

<div class="row justify-content-center">
    <div class="col-sm-10 mt-3 mt-md-0">
        {% include figure.liquid path="assets/img/llm-inference/static-batching.png" title="static batching" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Static batching. Left: the batch mid-flight. Right: S3 finishes at T5 and S1 at T6, but their slots sit empty (white) until the longest sequence, S2, finally finishes at T8; only then can the next batch start. Credits: <a href="https://www.anyscale.com/blog/continuous-batching-llm-inference">Anyscale, "How continuous batching enables 23x throughput in LLM inference"</a>.
</div>

<div class="row justify-content-center">
    <div class="col-sm-10 mt-3 mt-md-0">
        {% include figure.liquid path="assets/img/llm-inference/continuous-batching.png" title="continuous batching" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Continuous batching, same workload. The moment a sequence emits END, its slot is handed to a waiting request at the next step: S5, S6, and S7 begin their (yellow) prefills in slots that static batching left idle. No white cells. Credits: <a href="https://www.anyscale.com/blog/continuous-batching-llm-inference">Anyscale</a>.
</div>

Making one forward pass serve this mixed population takes one more idea, and it's Orca's second contribution: **selective batching**. The obstacle is that the sequences in a step are at different lengths and even different phases, one slot might be prefilling 512 prompt tokens while three others decode 1 token each, and tensors of such ragged shapes don't stack into the neat batched matmul that batching normally relies on. Orca's observation is that the ops in a transformer layer split into two classes, and only one of them cares. The linear layers, norms, and activations are *per-token*: they apply the same weights to each token's vector independently, with no interaction between tokens, so the raggedness is irrelevant; flatten all the tokens from all sequences into one long $$[\text{total tokens} \times d_{model}]$$ matrix and run them as a single fused matmul, exactly as if they were one sequence. Attention is the exception: each token must attend over *its own sequence's* K and V, of its own length, so sequences can't be flattened together there. Hence "selective": batch every op except attention across the flattened tokens, then split at the attention boundary and let each sequence attend against its own cache. This is precisely the shape of vLLM's fused step from the previous section, prompt chunks and decodes flattened into one forward pass, with the per-sequence work pushed into the attention kernel (the kernels section shows how PagedAttention handles the raggedness).

**Chunked prefill** addresses the tension continuous batching creates. Once prompts and decodes share steps, a 10K-token prompt entering the batch means one step must process 10K tokens instead of a few dozen, and every co-batched decode inherits that multi-hundred-millisecond step as a frozen token stream; this is exactly the ITL-tail pathology from the metrics section. Sarathi-Serve's fix ([Agrawal et al.](https://arxiv.org/abs/2403.02310)) is to stop treating a prompt as atomic. vLLM V1 has it on by default ([docs/configuration/optimization.md](https://github.com/vllm-project/vllm/blob/main/docs/configuration/optimization.md)), built from three interlocking pieces:

- **A per-step token budget.** Each step may run at most `max_num_batched_tokens` tokens through the fused forward pass (the numbers section's defaults: 2048 to 16384 depending on GPU and entrypoint). Since step time grows with tokens processed, capping tokens per step caps the duration of every step, and a bound on step time is a bound on every ITL sample: the budget is the knob that decides the worst-case gap any user can experience.
- **Decodes are scheduled first.** Within a step's budget, every running sequence gets its 1 decode token before any prompt gets anything. Decodes are tiny (a batch of 200 running requests consumes just 200 tokens of budget), so this is cheap, and it guarantees in-flight streams never stall behind a newcomer's prompt: their ITL is protected by construction.
- **Prefill gets the leftovers, in chunks.** Whatever budget remains after the decodes goes to waiting prompts, and a prompt larger than the leftover is *chunked*: it runs partway this step and continues next step. With a budget of 8192 and 200 running decodes, a 20K-token prompt gets 7992 tokens processed in its first step, then continues across the following steps until done; the KV cache accumulates across chunks exactly as it would have in one shot, so nothing is recomputed.

Put together: every step is decodes plus at most a budget's worth of prefill, so steps stay uniformly sized and ITL stays flat, while the long prompt's TTFT stretches over the few extra steps its chunks now take. The spike doesn't vanish; it's amortized, and the budget (plus a per-request chunk cap we'll meet in the scheduling section) sets the exchange rate between the long request's TTFT and everyone else's ITL.
