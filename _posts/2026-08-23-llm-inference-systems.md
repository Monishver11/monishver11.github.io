...
Before any more machinery, we need to fix what "fast" means. Every metric here is one of two kinds. **Latency** is one request's experience: how long did *this* user wait. **Throughput** is the operator's aggregate: how much work the whole system completes per second. The two pull against each other, and this section is about measuring each honestly.

One piece of vocabulary first: latency is a distribution over many samples, not a number, and **p99** names the value that 99% of samples fall below (so p99 ITL = 200 ms means 1 gap in 100 was worse than that). Serving is judged at p99 rather than the mean because a request generating 500 tokens samples the gap distribution 500 times: even a 1-in-100 stall hits essentially every long request, and the user feels the freeze, not the average. That is why the targets later in this section are stated at percentiles.

The definitions:

| Metric | Stands for | Definition | What to watch |
|---|---|---|---|
| TTFT | time to first token | request sent to first token received | queueing plus prefill; client-side it also includes tokenization |
| ITL | inter-token latency | gap between consecutive tokens | the tail (p99) exposes prefill insertions and preemptions |
| TPOT | time per output token | $$(\text{E2EL} - \text{TTFT}) / (\text{output tokens} - 1)$$ | a per-request average; hides stalls |
| E2EL | end-to-end latency | request sent to last token | $$= \text{TTFT} + \sum \text{ITL}$$ |
| Throughput | | tokens/s or requests/s completed | meaningless without a latency constraint |
| Goodput | | requests/s meeting **all** stated SLOs | the comparable headline number ([DistServe](https://arxiv.org/abs/2401.09670)) |

##### **ITL versus TPOT**

These two sound interchangeable and are not. ITL is a *sample per decode step*: every gap between consecutive tokens is one measurement, so ITL has a distribution and a meaningful p99. TPOT is a *per-request mean*: total decode time divided by tokens generated. If a request stalls once for 800 ms mid-generation, that stall is one enormous ITL sample, while TPOT smears it across a few hundred tokens and barely moves. When hunting scheduling pathologies, ITL p99 is the metric that talks.

And the classic pathology to keep in mind: **admitting a prompt helps that request's TTFT and hurts everyone else's ITL**, because its prefill tokens ride in the same forward pass as the in-flight decodes and stretch the step for all of them. Every scheduler decision in this post sits somewhere on that trade-off.

##### **Tokens/s versus requests/s**

The throughput row lists two units because they answer different questions. **Tokens/s** counts the engine's actual work (a 900-token generation is 100x the work of a 9-token one, and is weighted accordingly), which makes it the unit for comparing engines and kernels. **Requests/s** is what capacity planning buys: how many users one replica serves. Converting between them requires the workload's average tokens per request, so the two agree only on a fixed workload. One convention to check in any benchmark: whether "tokens" means output tokens only or prompt plus output; both get reported, and on prefill-heavy workloads the difference is large.

##### **Goodput**

Raw throughput has a flaw that makes benchmark headlines misleading: **any engine can buy tokens/s with batch size, at the cost of latency**. One decode step reads the weights once and yields one token per sequence, so a bigger batch means more tokens per step; but the step also gets slower as the batch grows, so every user's ITL worsens at the same time (a decoding user gets exactly one token per step, so their ITL *is* the step time, and each added sequence makes the step longer by adding its own KV reads, sampling work, and FLOPs on top of the fixed weight read). The same engine on the same GPU can report 3000 or 5000 tok/s depending on how much latency it is configured to inflict, which is why a tokens/s number with no latency constraint attached is not comparable across systems.

Goodput is the fix ([DistServe](https://arxiv.org/abs/2401.09670)): state the latency requirements first, then count only the requests that met *all* of them. With requirements "TTFT under 500 ms and TPOT under 50 ms," a request that finished but took 900 ms to first token counts **zero**. Now push batch size up: raw throughput keeps rising, but more and more requests miss their bounds and drop out of the count, so goodput rises, peaks, and then falls. That peak is the honest operating point, and goodput, together with the exact requirements behind it, is the number to ask for when someone quotes a benchmark.

##### **SLOs**

Those stated requirements are **SLOs** (service-level objectives); an SLO written into a contract with consequences is an **SLA**. For an LLM service they are typically a TTFT bound and an ITL or TPOT bound, stated at a percentile (p99 under some threshold, not the mean). The values come from the product, not the engine: interactive chat needs a fast first token and a stream that outpaces reading, while an offline pipeline may need no per-token bound at all and should simply run for raw throughput. Once SLOs are fixed, tuning has a concrete objective: raise batch and load until the bound percentile approaches its limit, which is exactly operating at maximum goodput. The scheduling section's knobs are the levers this procedure adjusts.
