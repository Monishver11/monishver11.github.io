...
|--------------------------|-----------------------|-----------------------------|
| Input combination        | $$ \theta^\top x $$  | $$ \theta^\top x $$         |
| Output                   | Real-valued          | Categorical (0 or 1)        |
| Conditional distribution | Gaussian             | Bernoulli                   |
| Transfer function        | Identity             | Logistic $$ f(\cdot) $$     |
| Mean                     | $$ \mathbb{E}[Y \mid X = x] = \theta^\top x $$ | $$ \mathbb{E}[Y \mid X = x] = f(\theta^\top x) $$ |  
...
2. **Modeling**:  
   - Choose a parametric family of distributions $$ p(y; \theta) $$.  
   - Select a transfer function that maps a linear predictor in $$ \mathbb{R} $$ to the appropriate range of the parameters.  
3. **Learning**: Use MLE to estimate the parameters:  
   $$
   \hat{\theta} = \arg\max_\theta \log p(D; \theta).
   $$  
4. **Inference**: For prediction, map $$ x $$ through the learned transfer function.  


In the next section, we’ll dive deeper into these generalized models, exploring their flexibility and application to diverse prediction tasks. Stay tuned!  
...
We assume $$ Y \mid X = x \sim \text{Poisson}(\eta) $$. The challenge is to ensure $$ \eta $$, the rate parameter, is positive. This is achieved using a transfer function $$ f $$:  

$$
x \mapsto w^\top x \quad \text{(linear predictor in } \mathbb{R} \text{)} \mapsto \eta = f(w^\top x) \quad \text{(rate parameter in } (0, \infty) \text{)}.
$$  
...
$$
\log p(D; w) = \sum_{i=1}^{n} \left[ y_i \log \lambda_i - \lambda_i - \log(y_i!) \right],
$$  

where $$ \lambda_i = e^{w^\top x_i} $$. Substituting $$ \lambda_i $$, we get:  
...
In the next section, we’ll explore how these models are applied in real-world scenarios and their limitations. Stay tuned!  
