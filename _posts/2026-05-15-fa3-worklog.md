...
**3-pass safe softmax.** Notation: $$m_i = \max_{j=1}^{i} x_j$$ with $$m_0 = -\infty$$; $$d_i = \sum_{j=1}^{i} e^{x_j - m_N}$$ with $$d_0 = 0$$ ($$d_N$$ is the safe-softmax denominator); $$a_i$$ is the final softmax value.
...
It still takes two passes. Can we reduce it to a single pass to minimize global I/O? Yes, but we'll see that in the FA2 section, once tiling is covered in the next section.
