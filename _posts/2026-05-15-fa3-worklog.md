...
As with $$d_i'$$, the $$N$$-th terms agree: $$o_N' = o_N$$, so computing $$o_N'$$ gives us the answer we want. (For why this surrogate substitution is legitimate, revisit the online-softmax section, the argument is identical to the one for $$d_i'$$.)
...
<!-- FIGURE: Figure 2 from the paper — the hardware mapping diagram. Blue blocks = tiles resident in SRAM, red blocks = the current row; L = sequence length (e.g. 16k), D = head dim (e.g. 128 for GPT-3), B = block size. -->

<!-- TODO: write this subsection. Key points to cover:
  - sweep direction: tiles swept left-to-right for K^T, top-to-bottom for V, with m/d/O states updated as we go.
  - the SRAM footprint depends only on B and D, NOT on L. This is the whole reason FlashAttention scales to long context: GPU shared memory is small (228 KB/SM on H100) and L can be huge (16k+), but the footprint never grows with L.
  - tie back to the memory/compute hierarchy section and the occupancy tradeoff.
-->

For now, the key takeaway from the hardware mapping: the SRAM footprint depends only on the block size $$b$$ and the head dimension $$d$$, never on the sequence length $$L$$. Since shared memory is small (228 KB/SM on H100) and $$L$$ can be very large (16k or more), this is exactly the property that lets FlashAttention scale to long context without running out of memory.

> Note: in this blog we only cover the forward pass, since inference is the main thing we want to cover. Training additionally needs the backward pass to propagate gradients and update parameters; for that, please refer to the paper.
