...
<!-- I'm not going to use LLMs for most of this content, for a few reasons.  NOTE: worth a one-liner on *why*? readers will be curious. That said, I'll make sure there aren't any conceptual mistakes. --> 
If you find any, please let me know and I'm happy to correct them and make this useful for everyone. If you're in this domain, feel free to ping me — I'm happy to collaborate on projects around GPU kernels and ML perf engineering.
...
We do not touch the query row. $$Q_k$$ stays whole. What we tile is the key/value *sequence dimension*: instead of walking it one row at a time, we walk it $$b$$ rows at a time. So $$K$$ and $$V$$ are each cut into $$N/b$$ tiles of $$b$$ consecutive rows, each tile of shape $$(b, d)$$, and the loop now runs $$N/b$$ times.

The consequence: the score $$x_i$$ is no longer a scalar. At step $$i$$ we dot the single query row against all $$b$$ key rows of tile $$i$$ at once, producing a **vector** of shape $$(1, b)$$:

$$
x_i = Q_k\, K_{[(i-1)b\,:\,ib]}^\top \quad \in \mathbb{R}^{1 \times b}
$$

We write $$x_i[j]$$ for the $$j$$-th entry of this vector, i.e. the query row dotted with the $$j$$-th key row *inside* tile $$i$$:

$$
x_i[j] = Q_k\, K_{(i-1)b + j}^\top, \qquad j = 1, \ldots, b
$$

The double index is just "which tile" ($$i$$) and "which row within the tile" ($$j$$). With that, the recurrence is the single-pass one, with the max and the two sums now running over the $$b$$ entries of the tile instead of a single scalar:

$$
\begin{aligned}
&\textbf{for } i = 1 \ldots N/b: \\
&\quad x_i = Q_k\, K_{[(i-1)b\,:\,ib]}^\top &&\in \mathbb{R}^{1 \times b} \\
...
&\quad o_i' = o_{i-1}'\, \dfrac{d_{i-1}'}{d_i'}\, e^{m_{i-1} - m_i} + \sum_{j=1}^{b} \dfrac{e^{x_i[j] - m_i}}{d_i'}\, V_{\,(i-1)b + j} \\[6pt]
&O_k = o_{N/b}'
\end{aligned}
$$

Here $$m_i^{(\text{local})}$$ is the max within tile $$i$$ only; $$m_i$$ folds it into the running global max, exactly as before. $$V_{(i-1)b+j}$$ is the $$j$$-th value row of tile $$i$$, shape $$(1, d)$$, and $$o_i'$$ stays shape $$(1, d)$$. Nothing in the algorithm changed except that one loop step now consumes a whole $$(b, d)$$ K-tile and V-tile, which is what lets each tile be loaded into SMEM once and fully consumed there.
...
- The score $$x_i$$ becomes a **matrix** of shape $$(B_r, b)$$: $$B_r$$ query rows against $$b$$ key rows. We write $$x_i[r, j]$$ for query row $$r$$ dotted with key row $$j$$ of tile $$i$$.
...
The scalar reductions become per-row reductions: the max over a tile becomes a `rowmax` (reduce the $$b$$ axis, one max per query row), and the sum becomes a `rowsum`. The recurrence is otherwise identical:

$$
\begin{aligned}
&\textbf{for } i = 1 \ldots N/b: \\
&\quad x_i = Q_{[\,\cdot\,]}\, K_{[(i-1)b\,:\,ib]}^\top &&\in \mathbb{R}^{B_r \times b} \\
...
&\quad o_i' = \text{diag}\!\left(\tfrac{d_{i-1}'}{d_i'}\, e^{m_{i-1} - m_i}\right) o_{i-1}' + \text{diag}\!\left(\tfrac{1}{d_i'}\right) e^{x_i - m_i}\, V_{[(i-1)b\,:\,ib]} \\[6pt]
&O_{[\,\cdot\,]} = o_{N/b}'
\end{aligned}
$$

A note on the shapes in the last two lines: $$m_i$$ is a $$(B_r,)$$ vector, so in $$x_i - m_i$$ it broadcasts across the $$b$$ columns of the $$(B_r, b)$$ score matrix. In the $$o_i'$$ update, the per-row scalars $$\frac{d_{i-1}'}{d_i'} e^{m_{i-1}-m_i}$$ and $$\frac{1}{d_i'}$$ multiply each query row of the output block, which is what $$\text{diag}(\cdot)$$ expresses, scaling row $$r$$ of the block by entry $$r$$ of the vector. There is **no new algebra here**: it is the Step 1 recurrence with a row index attached, made possible purely by the independence of query rows.

With both tilings in place, one loop step now processes a full $$(B_r, b)$$ score tile: a block of query rows against a block of key/value rows, with all running state held as small $$(B_r,)$$ vectors and one $$(B_r, d)$$ output block. This is the form that maps directly onto the GPU, which is the next subsection.
