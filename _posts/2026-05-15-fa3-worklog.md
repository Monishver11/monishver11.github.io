...
The computation of every output row is independent, so we explain the computation of one row for simplicity. We'll write everything for the $$k$$-th output row $$O[k, :]$$. We adopt the paper's bracket-index notation here so that shapes are unambiguous from the syntax itself:
...
- $$O[k, :]$$: the $$k$$-th row of the output matrix $$O$$. Shape $$(1, d)$$. This is what we want to compute.
- $$x_i$$: a scalar, the $$i$$-th attention score. Comes from a row times a column: $$x_i = Q[k, :] K^\top[:, i]$$.
- $$m_i, d_i'$$: scalars, the running max and running surrogate denominator from online softmax. $$d_i'$$ is reused here exactly as defined in the online-softmax section.
...
The bracket notation makes the shapes self-documenting: $$[k, :]$$ is a row, $$[:, i]$$ is a column, and operations between them follow the standard matrix rules. <!-- TODO: optionally, when an illustration is added, place it here to anchor what these row/column/scalar quantities look like geometrically. -->

<!-- TODO: notation in surrounding subsections (online softmax above, Step 1 and Step 2 tiling below) still uses bare-subscript form ($$Q_k$$, $$V_i$$, $$K_i$$, $$m_i$$). Bracket-index notation is scoped to this derivation only. If clarity becomes an issue later, propagate bracket notation to those sections too. -->

With those in hand, we can write the per-row attention computation as a recurrence. We reuse the 2-pass online softmax from before, and add a second loop that accumulates the output row.
