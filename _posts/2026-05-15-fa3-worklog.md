...
#### **K4.2: Full WGMMA with register handoff**

##### **What's new**

K4.1 ran QK on WGMMA, scattered $$S$$ to SMEM, ran the softmax there, then did PV scalar in registers. K4.2 puts PV on WGMMA too. That sounds like one change, but it forces a cascade:

1. PV's A operand is $$P$$, the exponentiated $$S$$. To use WGMMA on PV, $$P$$ has to live either in SMEM or in registers in the right per-thread layout. SMEM costs us a round-trip we'd rather skip; register-source A is the FA3 path, so we take it.
2. If $$P$$ lives in registers under the WGMMA A-layout for PV, then the softmax can no longer scatter $$S$$ to SMEM and read it back as a flat tile. The softmax must run **directly on the per-thread WGMMA C-fragment** of `acc_qk`.
3. Running the softmax on a register fragment means knowing exactly which fragment slots correspond to which $$(M, N)$$ tile positions, which rows are owned by which threads, and which threads share a row for the cross-thread reduction. That's the WGMMA C TV layout in full detail. K4.1 hid all of it behind the coordinate-tensor scatter; K4.2 has to learn it.
4. Once `acc_qk` holds $$P$$, we hand it to PV as the A operand. The handoff reinterprets the same physical registers under the PV A-layout instead of allocating a new fragment.
5. PV's B operand is $$V$$. $$V$$'s natural storage looks identical to $$K$$'s (both row-major with $$d$$ contiguous), but plays a different role in its matmul: for QK, $$K$$'s contiguous axis is the K axis; for PV, $$V$$'s contiguous axis is the N axis. WGMMA needs to know which, and "the same physical bytes" gives different answers depending on the matmul's perspective. We fix this by re-viewing $$V$$ with mode order $$(d, k, l)$$ instead of $$(k, d, l)$$.

Everything else stays the same: 128 threads, one warpgroup, 5 KV stages, swizzled SMEM, TMA loads.

##### **The blocker: why P-through-SMEM doesn't extend**

For the PV matmul, $$A = P$$. WGMMA constrains B to come from SMEM, but A can come from SMEM **or** registers. So in principle there are two options for $$P$$:

- **A from SMEM.** Write the softmax output to an SMEM tile `sP` in the right layout, then have PV read it back as A. This sounds analogous to K4.1's `acc_qk → sS` scatter and would let us reuse infrastructure.
- **A from registers.** Keep $$P$$ in the registers `acc_qk` already lives in, hand them off directly to PV.

The SMEM route looks tempting but doesn't actually work cleanly. The reason is that the WGMMA C TV layout (which `acc_qk` lives under) and the WGMMA A TV layout (which PV expects its A in) are *different*, and the conversion between them is per-thread register-level, not per-position SMEM-level. To do the conversion through SMEM you would need to write `acc_qk` to SMEM under one swizzled pattern, then read it back under a different swizzled pattern, hoping the bytes line up. They don't in general, and the byte-level mismatch is invisible to anyone looking at logical tile positions.

The register route avoids all of this. The SM90 WGMMA C and A register layouts are designed to be reinterpretable into one another *without inter-thread shuffles* for BF16, precisely so that back-to-back GEMMs like QK→PV can hand off through registers. That's the path K4.2 takes.
...
```python
tiled_mma.tv_layout_A   # how threads partition A
tiled_mma.tv_layout_B   # how threads partition B
tiled_mma.tv_layout_C   # how threads partition C
```

The crucial property of WGMMA C-layouts on SM90: **both the Thread mode and the Value mode contribute to both M and N of the tile**. Some sub-bits of the thread index walk you along M; others walk you along N. Same for the value index. This is why a single thread's fragment is not a clean sub-rectangle of the tile; it's scattered across the tile in a hardware-defined pattern.
...
128 threads ($$4 \times 32$$, one warpgroup of 4 warps × 32 lanes) each holding 32 values ($$2 \times 2 \times 8$$). The hierarchical decomposition isn't arbitrary; each sub-mode has a specific stride that walks you a specific distance through the tile. The exact strides are hardware-defined and live in `tv_layout_C.stride`.

Per thread, this resolves to:
...
Two things from this table determine the K4.2 softmax design:

- Each thread owns **2 distinct M rows**, not one. The per-row state arrays (`s_max`, `a_sum`, `s_max_prev`) must have length 2, not be a single scalar.
- Each M row is shared by **4 threads**. The row max and row sum must reduce across those 4 sibling threads, not across some hardcoded count.

K4.1's `tidx // 2` row mapping was correct only because we wrote `acc_qk` back to SMEM in tile shape first; the WGMMA scatter was dissolved by the round-trip. K4.2 can't do that anymore, so the softmax has to be built around the WGMMA C-layout's actual structure.

###### **The stride-versus-M test**

CuTe needs a way to peel apart "this sub-mode walks along M" from "this sub-mode walks along N" inside the hierarchical Thread and Value modes. The test is simple: picture the tile flattened M-major into a 1D buffer of $$M \cdot N$$ entries. Addresses $$0 \ldots M-1$$ form M-row 0, $$M \ldots 2M-1$$ form M-row 1, and so on. Then:

- A stride $$< M$$ moves within a single row → it walks along M.
- A stride $$\geq M$$ moves across rows → it walks along N.

So given a sub-mode with stride $$s$$, you classify it as "M-walking" if $$s < M$$ or "N-walking" otherwise. This works for both the thread mode and the value mode of the TV layout. Two utilities in K4.2 use this test: `layout_acc_mn` (peels the value mode) and `reduction_target_n` (peels the thread mode). Both call into a shared helper, `layout_separate`.

##### **`layout_separate`: peeling a hierarchical mode**
...
Given a layout `src` and a parallel list of strides `ref`, it splits `src`'s sub-modes into two buckets: those whose corresponding stride in `ref` is $$<$$ `thr` go into `lt` (the M-walking part); the rest go into `ge` (the N-walking part). In K4.2 `thr` is always `tiled_mma.shape_mnk[0]`, which is $$M$$.

This is the surgical tool: *given a hierarchical mode that mixes M and N contributions, separate it into (M-part, N-part)*.

##### **`layout_acc_mn`: a friendly mn-view of the fragment**

The per-thread C fragment naturally has shape `(value_mode, M_tile_coord, N_tile_coord)` — `acc[0]` is the value mode, `acc[1]` and `acc[2]` are the per-tile M and N coords if the matmul covers multiple atoms (in our case `atom_layout_mnk=(1,1,1)`, so `acc[1]` and `acc[2]` are trivially-sized). What we want is an mn-view where indexing `acc[i, j]` walks $$i$$ along distinct M rows owned by this thread and $$j$$ along distinct N columns.

`layout_acc_mn` builds that view:
...
def layout_acc_mn(self, tiled_mma, acc):
    ...
    V_M = separated[0]     # M-walking part of value mode
    V_N = separated[1]     # N-walking part of value mode
...
Indexing the result `acc_mn[i, j]`:
...
And `cute.size(acc_mn, mode=[0])` gives the number of distinct M rows this thread owns: 2 for the 64×64 atom.
...
These constants drive every loop in the K4.2 softmax.

##### **`reduction_target_n`: the threads that share a row**

A row max needs to reduce over **all** N columns of a fixed M row. Within one thread, the inner loop over `n_cols_qk` covers the 16 N columns this thread owns for each of its 2 rows. The remaining columns of that row live in **other threads' fragments** — specifically, the threads whose thread-id differs in the N-walking bits of the thread mode but agrees in the M-walking bits.
...
Same `layout_separate` machinery, applied this time to the **thread** mode (`tv_layout_C.shape[0]` and `tv_layout_C.stride[0]`). It returns the part of the thread index whose stride is $$\geq M$$, i.e. the bits that walk along N. Threads that differ only in those bits are siblings on the same row.

For our 64×64 atom this returns a 1-rank layout with shape `[4]`: 4 sibling threads per row, exactly what the table earlier predicted. The K4.2 reduction iterates over the sub-modes of this return value and does a sub-warp shuffle whose group size is each sub-mode's extent:

```python
reduction_target_qk = self.reduction_target_n(qk_tiled_mma)
red_rank_qk = cute.rank(reduction_target_qk)

# After computing local row maxes in registers:
...
After this loop, every thread that owns row $$i$$ holds the same global row-max in `s_max[i]`, and the per-row rescale of `acc_pv_mn[i, *]` becomes safe.

> **Why not just use `threads_in_group=4`?** Hardcoding 4 would work for the 64×64 atom we use, and would silently break for any other atom shape. The K4.2 code derives the group size from `reduction_target_n` so the same softmax works for any atom CuTe gives us.

##### **The register softmax in full**
...
`acc_pv` is allocated **once before the mainloop**, zero-initialised, and accumulates across KV iterations. It does not get reset per iteration.
...
Inside the mainloop, after QK has produced `acc_qk` and we have `acc_qk_mn` from `layout_acc_mn`, the softmax runs entirely on registers:
...
# 2. Causal mask (per-element, using the QK identity tensor)
if const_expr(causal):
    for i in range_constexpr(n_rows):
        for j in range_constexpr(n_cols_qk):
...
# 3. Save previous row max, take new row max over this block (local + cross-thread)
for i in range_constexpr(n_rows):
...
# 5. P = exp(S - s_max) in-place into acc_qk, accumulate row sum
for i in range_constexpr(n_rows):
    for j in range_constexpr(n_cols_qk):
...
Three things to note about this body:

- It's the same FA2 recurrence as K2/K3, just expressed against the per-thread fragment layout. The arithmetic is unchanged.
- Step 5 overwrites `acc_qk` in place with $$P$$. After this point `acc_qk` is no longer the score block; it's the (unnormalised) attention weights. We hand it to PV as A in the next step.
- The `a_sum` reduction is deferred to the finalize stage, after all KV iterations are done. We accumulate `a_sum[i]` per KV iteration on the local-N-columns this thread owns, then do one cross-thread reduce at the end. This is FA3's "tail" pattern: keep the partial sums per-thread until they're needed.

##### **The QK → PV register handoff: `make_acc_into_op`**

After the softmax, `acc_qk` holds $$P$$ in FP32 under the WGMMA C TV layout. PV wants its A operand in registers, in BF16, under the WGMMA A TV layout. The handoff has to do three things: cast FP32 → BF16, reinterpret the register layout C → A, and avoid moving bytes between threads (since that would cost us shuffles).
...
**Step 1: allocate the A-layout register fragment.**
...
`convert_c_layout_to_a_layout` reshapes the C-fragment layout into the A-fragment layout that physically aligns with how WGMMA expects A for back-to-back GEMM. This works because the SM90 WGMMA register layouts for C and A are designed to be reinterpretable into one another *without inter-thread shuffles for BF16*, precisely to make QK→PV through registers efficient.
...
This is a CUTLASS C++ idiom ported into Python; the precise stride/shape arithmetic isn't load-bearing for understanding. The takeaway is that the output describes an A-shaped layout pointing at the *same physical registers* as the input C-shaped layout. The compiler can therefore reinterpret in place.
...
Same iterator (same physical registers as `operand`), C-shaped layout (same as `acc`). One register region, two views: `operand` (A-shaped, BF16) and `operand_as_acc` (C-shaped, BF16). Indexing through `operand_as_acc[c_coord]` lands on the same register as indexing through `operand[a_coord]` where `a_coord` is the A-layout coordinate that maps to the same physical register.
...
Reading is C-shaped because `acc` is C-shaped. Writing routes the FP32→BF16 cast into the same registers that will be read by WGMMA under the A-layout (because `operand_as_acc` and `operand` are aliases). The cast happens in registers, no SMEM, no shuffles.

The function returns `operand`, the A-shaped BF16 fragment, which is what PV's `cute.gemm` expects.

> **FP8 caveat.** The "no inter-thread shuffles" property holds for BF16. FP8 (E4M3 etc.) breaks it because the per-element bit width changes the per-thread register packing, and the cleanest BF16 reinterpretation no longer maps the right bytes to the right threads. K7 will need extra shuffles in `make_acc_into_op`. Flagging here for narrative continuity.

##### **V mode-order: the major-mode trap (callout)**
...
>
> For **QK**, $$K$$ is the B operand and the contraction is $$d$$ (we sum $$Q_{:,k} K_{:,k}^\top$$ over $$k = 0 \ldots d-1$$). $$K$$'s contiguous axis is $$d$$, which is QK's K axis. So $$K$$ is K-major.
>
> For **PV**, $$V$$ is the B operand and the contraction is $$B_c$$ (we sum $$P_{:,k} V_{k,:}$$ over $$k = 0 \ldots B_c-1$$). $$V$$'s contiguous axis is still $$d$$, but for PV that's the N axis, not the K axis. So $$V$$ needs to be **MN-major** for PV.
>
...
With V re-viewed, the PV tiled MMA is straightforward:
...
- The M/N of the tile is `(Br, d)` instead of `(Br, Bc)`. The PV result is $$(B_r, d)$$, matching the shape of $$O_i$$.
- `OperandSource.RMEM` for A. WGMMA reads A from registers, so we will pass the BF16 fragment returned by `make_acc_into_op` directly.
...
Inside the mainloop, after the softmax has converted `acc_qk` to $$P$$, the PV matmul looks structurally like the QK matmul but with A from registers:
...
**`ACCUMULATE` is always `True`.** `acc_pv` is the running attention output across all KV iterations. Every PV WGMMA adds to whatever `acc_pv` already holds. On iter 0 it adds to the zero-initialised state; on later iters it adds to the previously rescaled output. There's no "first call writes the accumulator" case for PV.

**A comes from registers.** `tOrP` is the BF16 A-layout fragment we just built via `make_acc_into_op`. It's the same physical registers as `acc_qk`, reinterpreted. The k_block axis lives in `tOrP`'s mode 2: `num_k_blocks_pv = cute.size(tOrP, mode=[2])`. For $$B_c = 64$$ and atom $$K = 16$$, that's 4 k_blocks.

**The PV fence applies to the A operand.** The QK fence in K4.1 was largely cosmetic (no register operands feeding QK). The PV fence here is functional: it marks the registers in `tOrP` as ready to be consumed by WGMMA, because they were just written by the softmax / `make_acc_into_op` cast.

##### **Finalize**

Two pieces.

**Cross-thread `a_sum` reduce.** Throughout the mainloop each thread accumulated `a_sum[i]` over only the N columns it owns for row $$i$$. The actual row sum requires combining with the 3 sibling threads on the same row. We do this exactly once, at the end of all KV iterations, using `reduction_target_n` on the PV tiled MMA:
...
**Output scatter with a coord tensor.** `acc_pv` is in registers under the WGMMA C TV layout, same structural problem as K4.1's `acc_qk`. The same fix applies: probe with an identity tensor partitioned through `partition_C`, then write to GMEM at the per-thread tile coordinates.
...
K3.3 carried `kv_stage = 5` more as scaffolding than as a real optimisation: the K3 consumer was so much slower than the TMA loader that even one stage of prefetch was sufficient. K4.2 changes this calculus.

The PV WGMMA is now the consumer side. A single PV call on an $$N = d$$ atom is roughly an order of magnitude faster (in wall time) than scalar PV was. The QK WGMMA is similarly fast. Combined with the register softmax (no SMEM round-trip), one full KV iteration of compute is now genuinely fast. TMA latency, by contrast, hasn't changed: it's still bound by HBM bandwidth and SM cache fills.

So the ratio (compute time per iter) / (TMA time per iter) shifts dramatically toward compute being roughly comparable to TMA. To hide TMA latency, we need the producer to be working on the next tile while the consumer works on the current one, which is exactly what multi-stage pipelining buys.

Why specifically 5? Two bounds:

**Latency-hiding bound.** With $$N$$ stages the producer can run $$N - 1$$ iterations ahead. TMA round-trip latency on H100 plus consumer-rate variance means roughly 3-4 iterations of headroom is comfortable, with 5 giving margin.

**SMEM bound.** Each KV stage costs $$2 B_c d$$ bytes (K + V), so 16 KB per stage at $$B_c = 64$$, $$d = 128$$. 5 stages = 80 KB. Plus Q (16 KB), plus mbarriers, total around 100 KB. Comfortably under H100's 228 KB/SM with room for other resources.

K5 onwards will revisit pipeline depth as warp specialization and ping-pong scheduling change the consumer / producer rate ratio further.

##### **New CuTe primitives in K4.2**

- `cute.make_rmem_tensor_like(layout, dtype)` — allocate a register tensor with the given layout and dtype.
...
- `tensor.load()` / `tensor.store(vec)` — bulk read/write of a register tensor as a single value vector (used in `make_acc_into_op` for the cast).
- `cute.rank(layout)` — number of top-level modes; used to drive the `reduction_target_n` loop generically.
- `cute.size(tensor, mode=[k])` — extent of mode $$k$$; used everywhere for `n_rows`, `n_cols_*`, k_block counts.

The utility helpers (`layout_separate`, `layout_acc_mn`, `reduction_target_n`, `convert_c_layout_to_a_layout`, `make_acc_into_op`) aren't built-in CuTe primitives; they're project-level helpers built from the primitives above. They're load-bearing for K4.2 and reused by K5, K6, K7.

##### **Running K4.2**

```bash
CHECK=1 SEQLEN=512 HEADDIM=64 CAUSAL=0 python bench.py k4_2
```

##### **What we expect**

K4.2 is the first kernel in this work where both matmuls run on tensor cores and the on-chip data flow uses no avoidable SMEM round-trips. Expectations:

- TFLOPS jump significantly above K4.1, scaling with sequence length and headdim.
- The kernel should approach a meaningful fraction of H100's BF16 tensor-core peak (990 TFLOPS). The exact fraction depends on how well TMA latency is hidden, register pressure, and softmax overhead.
- NCU should show tensor-core utilisation as a substantial fraction of total math; the FMA pipe should be largely idle except for the softmax exp / reciprocal.

##### **Profiling**

<!-- TODO: NCU output for K4.2 -->

##### **What's next**

K4.2 leaves a few obvious things on the table. The QK and PV WGMMAs issue serially in each KV iteration, with the softmax sequenced strictly between them. The same 128 threads do load issuance (warp 0), compute, and store, which means they spend cycles on pipeline housekeeping instead of math. K5 introduces **warp specialization**: split the warpgroup into "load" warps (just issue TMAs) and "compute" warps (just issue WGMMAs and softmax). K6 then introduces **ping-pong scheduling** where two compute groups alternate so one runs WGMMA while the other runs softmax, hiding the softmax cost behind GEMM.
