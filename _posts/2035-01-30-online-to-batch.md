...
The **regret** of the algorithm is given by:

$$
R_T = \sum_{t=1}^{T} L(h_t(x_t), y_t) - \min_{h \in H} \sum_{t=1}^{T} L(h(x_t), y_t)
$$

Meanwhile, the **generalization error** of a hypothesis $$ h \in H $$ is its expected loss:
...
Our goal is to bound the **average generalization error** of the sequence of hypotheses generated by $$ A $$, using their **average loss** on the dataset $$ S $$.
